  ...
  const [addBook, { loading, error, data }] = useMutation(ADD_BOOK, {
   ...
   useSubscription(BOOK_ADDED, {
    onSubscriptionData: ({ client, subscriptionData }) => {
      console.log('NEWBOOK::Subscription Data:', subscriptionData); 
      const newBook = subscriptionData.data.bookAdded;
      window.alert(`Nuevo libro agregado: ${newBook.title} por ${newBook.author.name}`);
    },
    onError: (error) => {
      console.error("Error en la suscripción:", error); // Log para errores
    },
    onData: ({ data }) => {
