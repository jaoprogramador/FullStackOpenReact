...
const PatientDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      try {
        const patientResponse = await axios.get(`${config.apiUrl}/api/patients/${id}`);
        //const patientResponse = await axios.get(`http://localhost:3001/api/patients/${id}`);
        //setPatient(patientResponse.data);
        setPatient({ ...patientResponse.data, entries: patientResponse.data.entries || [] });
        const diagnosesResponse = await axios.get(`${config.apiUrl}/api/diagnoses`);
        //const diagnosesResponse = await axios.get('http://localhost:3001/api/diagnoses');
  ...
  const handleAddEntry = async (newEntry: Entry) => {
    try {
      const response = await axios.post(`${config.apiUrl}/api/patients/${id}`);
